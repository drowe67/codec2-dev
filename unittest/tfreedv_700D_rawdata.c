/*---------------------------------------------------------------------------*\
  ...
  FILE........: tfreedv_700D_rawdata.c
  AUTHOR......: Jeroen Vreeken
  DATE CREATED: 24 May 2020

  FreeDV 700D rawdata test.
...
int main(int argc, char **argv)
{
    ...
    printf("freedv_api tests for mode 700D\n");

    printf("freedv_open(FREEDV_MODE_700D) ");
    ...
    assert(frame_bits == 112);
    printf("%d Passed\n", frame_bits);

    printf("freedv_get_n_bytes_per_modem_frame() ");
    int frame_bytes = freedv_get_bytes_per_modem_frame(f);
    assert(frame_bytes == 14);
    printf("%d Passed\n", frame_bytes);

    printf("freedv_get_n_codec_frames() ");
    int n_codec_frames = freedv_get_n_codec_frames(f);
    assert(n_codec_frames == 4);
    printf("%d Passed\n", n_codec_frames);

    unsigned char payload[16] = { 0x12, 0x34, 0x56, 0x70, 0x89, 0xab, 0xcd, 0xe0, 0xf1, 0x23, 0x45, 0x60, 0x78, 0x9a, 0xbc, 0xd0 };
    unsigned char payload_tx[14] = { 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf1, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd };

    printf("freedv_codec_frames_from_rawdata() ");
    unsigned char codec_frames[16] = { 0 };
