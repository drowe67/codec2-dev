...
int main(int argc, char *argv[]) {
  ...
  while (o != -1) {
    ...
    help:
      ...
      fprintf(stderr, "\n");
      fprintf(stderr, "--lower lowermeanLimit   Only count vectors with "
                      "average above this level in distortion calculations\n");
      fprintf(stderr, "--mbest N                number of survivors at each "
                      "stage, set to 1 for standard VQ search\n");
      ...
      fprintf(stderr, "--num   numToProcess     number of vectors to quantise "
                      "(default to EOF)\n");
      fprintf(stderr, "--vec_usage              Output a record of how many "
                      "times each vector is used\n");
  ...
  while (fread(&target, sizeof(float), k, stdin) && (nvecs < num)) {
    ...
    if (verbose) {
      for (int i = 0; i < num_stages; i++)
        fprintf(stderr, "index: %d ", indexes[i]);
      fprintf(stderr, "\n");
    }
  ...
  fprintf(stderr, "var: %6.4f  sd: %6.4f\n", var, sqrt(var));
