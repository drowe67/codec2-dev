...
int main(int argc, char *argv[]) {
  ...
  int rateK = 0, newamp1vq = 0, rate_K_dec = 0, perframe = 0,
      postfilter_newamp1_en = 0, rateKlf = 0, setK = 0;
    ...
    case 0:
      if (strcmp(long_options[option_index].name, "Fs") == 0) {
        ...
        fprintf(stderr, "rateKout: each record is %d bytes\n",
                (int)(K * sizeof(float)));
      } else if (strcmp(long_options[option_index].name, "rateKnomeanout") ==
                 0) {
        ...
        fprintf(stderr, "rateKin: each record is %d bytes\n",
                (int)(K * sizeof(float)));
      } else if (strcmp(long_options[option_index].name, "rateKnomeanin") ==
                 0) {
        ...
        fprintf(stderr, "rateKnomeanin: each record is %d bytes\n",
                (int)(K * sizeof(float)));
      } else if (strcmp(long_options[option_index].name, "rateK_mean_min") ==
                 0) {
        rateK_mean_min = atof(optarg);
        fprintf(stderr, "rateK_mean_min: %f\n", rateK_mean_min);
      } else if (strcmp(long_options[option_index].name, "rateK_mean_max") ==
                 0) {
        rateK_mean_max = atof(optarg);
        fprintf(stderr, "rateK_mean_max: %f\n", rateK_mean_max);
      } else if (strcmp(long_options[option_index].name, "rateK_mean_comp") ==
                 0) {
        ...
        if (!lpc_model) {
          fprintf(
              stderr,
              "needs --lpc [order] to resample amplitudes when using --dec\n");
          exit(1);
        }
        ...
        if ((frateKWov = fopen(optarg, "wb")) == NULL) {
          ...
          exit(1);
                 ...
                 0) {
        /* dump 10ms of audio centred on analysis frame to check time alignment
           with 16 kHz source audio */
        ten_ms_centre = 1;
  ...
  fftr_fwd_cfg = codec2_fftr_alloc(FFT_ENC, 0, NULL,
                                   NULL); /* fwd FFT,used in several places   */
  fft_inv_cfg = codec2_fft_alloc(FFT_DEC, 1, NULL,
  ...
  while (fread(buf, sizeof(short), N_SAMP, fin)) {
    ...
    if (comp_en) {
      ...
      /*fprintf(stderr, "f: %d s_max_low: %6.0f s_max_high: %6.0f gains: %3.2f
      ...
      // limit dynamic range
      for (int m = 1; m <= model.L; m++)
        if (model.A[m] < mx * comp_dr_lin)
          model.A[m] = mx * comp_dr_lin;
    }
    ...
    if (rateK) {
      ...
      if (rateK_mean_comp_en) {
        ...
        if (mean > rateKmeancomp.thresh)
          mean = rateKmeancomp.thresh +
                 (mean - rateKmeancomp.thresh) * rateKmeancomp.slope;
        mean += rateKmeancomp.makeup_gain;
      }

      /* clip (limit) mean */
      if (mean < rateK_mean_min)
        mean = rateK_mean_min;
      if (mean > rateK_mean_max)
        mean = rateK_mean_max;

      if (frateKmean != NULL)
        assert(fwrite(&mean, sizeof(float), 1, frateKmean) == 1);
      ...
      if (frateKnomeanin != NULL) {
        assert(fread(rate_K_vec_no_mean, sizeof(float), K, frateKnomeanin) ==
               K);
      }
      ...
      if (postfilter_newamp1_en)
        post_filter_newamp1(rate_K_vec_no_mean_, rate_K_sample_freqs_kHz, K,
                            1.5);
      for (int k = 0; k < K; k++)
    ...
    */
    ...
    if ((frames % decimate) == 0) {
      ...
      for (i = 0; i < decimate; i++) {
        ...
        synth_one_frame(n_samp, fft_inv_cfg, buf, &model_dec[i], Sn_, Pn,
                        prede || de, &de_mem, gainoutlin);
#ifdef DUMP
  ...
  if (frateKin != NULL)
    fclose(frateKin);
  if (frateKnomeanin != NULL)
    fclose(frateKnomeanin);
  if (ften_ms_centre != NULL)
    fclose(ften_ms_centre);
  if (fmodelout != NULL)
    fclose(fmodelout);
  if (frateKWov != NULL)
    fclose(frateKWov);
