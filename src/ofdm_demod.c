...
int main(int argc, char *argv[]) {
    ...
    while ((fread(rx_scaled, sizeof (short), nin_frame, fin) == nin_frame) && !finish) {
        ...
        if ((ofdm->sync_state == synced) || (ofdm->sync_state == trial)) {
            ...
            if (llr_en == true) {
                ...
                if (ldpc_en) {
                    ...
                    if (testframes == true) {
                        Terrs += count_uncoded_errors(&ldpc, ofdm_config, codeword_symbols);
                        Tbits += coded_bits_per_frame; /* not counting errors in txt bits */
                    }

                    symbols_to_llrs(llr, codeword_symbols, codeword_amps, EsNo, ofdm->mean_amp, coded_syms_per_frame);                           
        ...
        if (verbose >= 2) {
           fprintf(stderr, "%3d nin: %4d st: %-6s euw: %2d %1d f: %5.1f pbw: %d, iter: %3d pcc: %3d\n",
                    f, nin_frame, 
