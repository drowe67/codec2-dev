...
int count_uncoded_errors(struct LDPC *ldpc, struct OFDM_CONFIG *config, COMP codeword_symbols_de[]) {
    int i;
...
void ofdm_ldpc_tx(struct OFDM *ofdm, struct LDPC *ldpc, complex float tx_sams[], uint8_t tx_bits[], uint8_t txt_bits[], struct OFDM_CONFIG *config) {
    int coded_syms_per_frame = ldpc->coded_syms_per_frame;
    int coded_bits_per_frame = ldpc->coded_bits_per_frame;
    int data_bits_per_frame = ldpc->data_bits_per_frame;
    ...
    ofdm_assemble_qpsk_modem_frame_symbols(ofdm, tx_symbols, coded_symbols, txt_bits);
