...
\end{equation}
where $t_k^j$ is the target, $f_k$ is the filter, $g^j$ the scalar gain (mean) added to the target, and $v_k^j$ the chosen VQ entry.  Solving for $\mathbf{f}^j$ is a highly non-linear problem, as $g^j$ is a function of $t_k^j$ and $f_k^j$; and $v_k^j$ is a function of the preceding three variables.  However if can solved by gradient descent:
\begin{equation}
\frac{\partial e_k^j}{\partial f_k^j} = -2(t_k^j - f_k^j -g^j - v_k^j)
\end{equation}
We then adjust $f_k^j$ to move in the opposite direction to the gradient:
\begin{equation}
f_k^{j+1} = f_k^j + 2 \Delta (t_k^j - f_k^j -g^j - v_k^j)
\end{equation}
where $\Delta$ is some small constant.  The procedure them becomes:
\begin{itemize}
\item At iteration $j$, we have $\mathbf{f}^j$ and $\mathbf{t}^j$.
...
\end{equation}
\begin{equation}
\mathbf{u} = \mathbf{t}^j - \mathbf{f}_i -g^j
\end{equation}
...
\begin{equation}
\mathbf{f}^{j+1} = \mathbf{f}^j + 2 \Delta (\mathbf{t}^j - \mathbf{f}^j - g^j - \mathbf{v}_{imin}^j)
\end{equation}
\end{itemize}
