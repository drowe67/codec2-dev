...
When $K$ > $L$, there are many solutions that result in a low $E$ fit.

Unlike a low pitch male (can we define by L?) peak doesn't correspond to formant positions.  Spectral envelope doesn't correspond directly to vocal tract. This means for the same formant uttered by a male and female, the vector required to represnet it may be different.  Fault in splitting the spectrally relevant information. Is this relevant?  Also width quite wide compared to males.

Implies quantisation of all points equally important, not just peaks.  DIfferent spectral envelop fit for say LPC (which works in the time domain). It woudl also find pitch dependnat solutions.

Narrowness is required for males to prevent muffling. So VQ is carrying pitch information
