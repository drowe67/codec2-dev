...
The input speech may be subject to arbitrary filtering, for example due to the microphone frequency response, room acoustics, and anti-aliasing filter.  This filtering is fixed or slowly time varying.  The filtering biases the target vectors away from the VQ training material, resulting in significant additional distortion.  The filtering does not affect the input speech intelligibility, only the VQ performance suffers.

Consider the $k$th element of the error vector $\mathbf{e}^j$ for the $j$th target vector:
\begin{equation}
e_k^j=(t_k^j - f_k^j -g^j - v_k^j)^2
\end{equation}
where $t_k^j$ is the target, $f_k$ is an estimate of the filter, $g^j$ the scalar gain (mean) added to the target, and $v_k^j$ the chosen VQ entry.  Solving for $\mathbf{f}^j$ is a highly non-linear problem, as $g^j$ is a function of $t_k^j$ and $f_k^j$; and $v_k^j$ is a function of the preceding three variables.  However if can solved by gradient descent:
\begin{equation}
...
TODO: scatter plot, in nice postscript form
