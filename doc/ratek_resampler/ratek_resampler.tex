...
Figure \ref{fig:plphase2_big_dog_61_time} illustrates the algorithm in action in the time domain, and Figure \ref{fig:plphase2_big_dog_61_freq} shows the effect in the frequency domain.  Compared to the \emph{phase0} algorithm alone, the algorithm better distributes the energy of each pitch cycle in time, which can be objectively measured by a reduction in PAPR. We note that the post filtered signal envelope is more symmetric in time than the original which shows the classic damped exponential envelope.  This may not matter to the ear, as our theory suggests it is the PAPR that matters, not the shape of the time domain envelope.
...
The second step involves an additional weighting of the input to the Hilbert Transform used to calculate the phase spectra:
\begin{equation}
\mathbf{\Omega}=HT(1.5*\hat{\mathbf{y}})
\end{equation}
where $HT()$ is a the Hilbert Transform function that returns a vector of phase samples $\mathbf{\Omega}$ from a vector of amplitude samples $\mathbf{y}$.  This was found to provide a further dispersion of the time domain energy, without affecting the magnitude spectrum.
...
\end{split}
\end{equation}
CPAPR tracks the time domain magnitude envelope of the signal.

\subsection{Further Work}

We now have a better understanding of how the post filter improves speech quality:
\begin{enumerate}
\item It increases the formant/anti-formant ratio, better defining formant locations to the ear, and hence increasing intelligibility. This helps improve speech quality for males and females.
\item For low pitch males, it distributes the time domain speech energy more equally in time across the pitch period than the minimum phase \emph{phase0} model alone, increasing the probability of detection of formant energy by the ear, reducing muffling and increasing intelligibility.
\end{enumerate}
...
For low frequency harmonics, it is important to have phase continuity between frames to prevent roughness.  The algorithm does not currently track or modify phase continuity between adjacent frames.
